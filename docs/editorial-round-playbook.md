...
### Running a round now that the site is live (2026-09-24)

The always-on server behind the domain serves the **public build**, which has
no workspace routes at all (`docs/deployment.md` → "The public build is what
is served"). A round therefore runs against a separate `next dev`, pointed at
the **live** data directory so its writes show on the site immediately — the
public pages are `force-dynamic` and read `config/` per request.

From a git worktree (where there is no `.env.local`), put exactly one line in
the worktree's git-ignored `.env.local`:

```
LOCAL_DATA_DIR=C:/Users/Administrator/ai-tech-radar/config
```

Nothing else goes there: the workspace guard stays off because the dev server
is on localhost only, and the LLM key is not copied. Model triage suggestions
are generated by a script that loads the main checkout's `.env.local` into its
own process — the same pattern as `EVAL_ENV_FILE` for the eval scripts — and
calls `generateTriageSuggestionsForUndecided()`; the key is never printed.
Delete the worktree's `.env.local` when the round is done, and commit the
round's `config/` changes from the live directory.
...
**Using the model's suggestions** (first used 2026-09-24; numbers in
`eval/triage/README.md`). They are advisory, and the two measurements so far
agree on how far to trust them:

- A **reject at confidence ≥ 0.85** has so far always matched the editor's
  call — 18 of 18 in the first live round, and no publish among 129
  high-confidence rejects in the test split. Skim the title and take it.
- A **"review" held because the summary is short is not a judgment about the
  item.** It means the feed carried a sentence. Six of the seven signals
  published in the first live round were in that bucket; every one was decided
  on the primary source. Read those articles.
- A **publish suggestion** is right about one time in six in the test split.
  Treat it as "worth opening", not as a decision.
...
**Check where the lines end, at 390px as well as 1265px** (added 2026-09-24).
Line count is not enough: a title can fit in three lines and still break
inside a word, because the segmenter splits common words — 上传, 重启, 权重,
价格战 — into single characters, and single characters are not kept together.
Walk the rendered `h1` character by character with `Range.getClientRects()`,
record each place the top coordinate jumps, and read the characters either
side. Three of seven titles in the 2026-09-24 round broke inside a word, two
of them only at 390px. As with line count, calibrate the simulator on the
original titles first — it must reproduce their real breaks — then reword
until every break falls between words at both widths.
