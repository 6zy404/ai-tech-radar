...
> 每个页面与视图零横向溢出，明暗两套。
> **等 owner 拍板的四条文案发现（同一份筛查，未动）**：搜索页在第一次搜索之前
> 只有四句话、其中三句在说同一件事；关系网络页的操作说明写了两遍（标题下的描述 +
> 图例面板）；首页「优先技术信号」区右侧放的是「网站怎么运作」的解释，而同页另外
> 三个区块的同一位置放的是短链接；简报「来源参考」6 张卡片只有 2 种标题，其中
> 一对逐字相同，而真正能区分它们的机构名被降级到副标题。
