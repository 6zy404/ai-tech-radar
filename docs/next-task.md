...
> **⑥ 本轮没有看图，如实写明。** 内置截图工具仍不可用（浏览器面板未显示、页面
> 不出帧），所以上面每一条都是**测量**结论。而最近两轮恰恰证明了：测量全绿的
> 同时，缺陷可以就摆在页面上。想补这一步需要授权用 Playwright + 本机 Chrome。
> 验证：typecheck / lint / format / vitest 188/188，两页结构回读确认无残留空标题，
> 各宽度无横向溢出，console 干净。
