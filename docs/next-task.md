...
> **量过但刻意不修，需要你定：`/network` 已经从 07-15 调优时的 33 个节点长到 94 个节点、
> 504 条边。** 那一轮改成圆点并复测到零压叠，现在被内容增长吃回去了：**1440 下 7 对压叠、
> 390 下 44 对**（画布只有 305px）。没坏——两个宽度下 0 个节点越出画布，点云占画布
> 85%×80%，无溢出——但修它要动那套手写力导向模拟（画布随节点数缩放，或按宽度调点径与
> 斥力），超出一次看图该决定的范围。
