...
> **看图产生假设，不能用看图关闭假设。**
> **筛查清单剩余 10 条（owner 已选下一批 = B 组）：**
> **B 组（对齐/构图，5 条）** —— 首页「技能」「知识」两栏第一张卡差 116px（左栏
> 3 张被拉开撑满右栏 4 张的高度）；三个详情页侧栏在页面 1/4 处结束、右侧 73–85%
> 高度空着（现在四页共用骨架，只需定一次规则）；首页「今日快讯」四条是纯白无边框卡
> 而同页其它都是纸面卡（07-14 迁移时因要 `target="_blank"` 而保留旧结构）；
> `/network` 的搜索+筛选控件悬空（**与 `/technologies` 上轮已修的完全同一个问题**，
> 当时按页逐个修、没回头找同类）；简报页「今日概览」是左标签右正文，左边 200px
> 空九行，而同页其余六块都是标题在上。
> **C 组（一致性，5 条）** —— 列表页 hero 两种渐变（`/technologies` 径向、其余 7 个
