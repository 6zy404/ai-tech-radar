...
- `id`
- `name`
- `purpose`: `editorial_enrichment` or `technology_comparison`
- `version`
...
purpose (Editorial Enrichment suggestions, or public technology comparisons).
Each generated record stores `promptVersionId` and the prompt `version` string
so the prompt that produced it can be traced. `getActivePromptVersion(purpose)`
falls back to a purpose-specific default (`defaultEditorialEnrichmentPromptVersion`
or `defaultTechnologyComparisonPromptVersion`) when no active version exists in
