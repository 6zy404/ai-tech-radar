...
## Public LLM Feature Boundary (Technology Comparison v0 + Explanation v1)

`POST /api/technologies/compare` was the first public route that calls the LLM
provider directly; `POST /api/technologies/explain` is the second and follows
the identical discipline. Both are intentionally left outside `middleware.ts`'s
protected path prefixes rather than gated behind `WORKSPACE_ACCESS_TOKEN`,
because:
...
- results are cached before any provider call: per unordered technology-id
  pair for compare (`config/technology-comparisons.json`) and per
  technology × reader level for explain
  (`config/technology-explanations.json`, cache key
  `technologyId::audienceLevel`), so each cache key is generated by the
...
- generated output is shown to readers **without editorial review**, unlike
  Editorial Enrichment's suggest → apply flow. Both responses always carry a
...
control in this prototype. Note that explain multiplies the cache space by the
three reader levels (published technologies × 3), which is still bounded. This
is acceptable for a local-first, mock-provider-by-default prototype but must
be revisited before any real-cost LLM provider is configured for a publicly
