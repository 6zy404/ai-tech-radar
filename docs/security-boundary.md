...
## Public LLM Feature Boundary (Technology Comparison v0)

`POST /api/technologies/compare` is the first public route that calls the LLM
provider directly. It is intentionally left outside `middleware.ts`'s protected
path prefixes rather than gated behind `WORKSPACE_ACCESS_TOKEN`, because:

- it only ever operates on technologies already returned by `getAllTechnologies()`,
  which is published-only — there is no way to reach draft or archived content
  through this route.
- results are cached per unordered technology-id pair
  (`config/technology-comparisons.json`), so the same pair is generated by the
  provider at most once; repeated requests are served from cache with no new
  provider call.
- the response is mapped through a single public-mapping function
  (`toPublicComparisonResult` in `src/lib/technology-comparison.ts`) that strips
  `providerName`, `modelName`, `promptVersionId`, `promptVersion`,
  `generationMode`, `outputValidationWarnings`, and `generationError` before the
  JSON ever leaves the server — the same internal-field discipline the LLM
  Provider Boundary above already requires, just enforced at a public rather
  than workspace-only response for the first time.
- generated comparisons are shown to readers **without editorial review**, unlike
  Editorial Enrichment's suggest → apply flow. The response always carries a
  `disclaimer` field, and the client widget renders it in the same paint as the
  result so unlabeled AI content is never visible.

Production-grade rate limiting for this route is not implemented (see
"public LLM route rate limiting" below) — the per-pair cache is the only cost
control in this prototype. This is acceptable for a local-first, mock-provider-
by-default prototype but must be revisited before any real-cost LLM provider is
configured for a publicly reachable deployment.
