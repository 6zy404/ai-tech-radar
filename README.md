...
- **用户端**：首页、技术信号列表（精选 / 全部快讯 / 按话题 / 我关注的 / 稍后读）、
  信号详情、技术简报与本周回顾、技能、知识、关系网络、话题页、站内搜索、问雷达（基于站内内容、
  带出处的问答）、RSS / JSON 订阅。只读取已发布内容。
...
外部信源（RSS / Atom / GitHub Release / 官方博客）
  ...
  → 发布检查（阻塞项 / 警告项）→ 技术信号
  → 技术简报 → 页面、RSS / JSON、投递渠道
```

候选池在编辑处置之前会以「自动聚合，未经编辑精选」的标注出现在快讯视图中；精选信号
与简报只包含经过编辑发布的内容。

截至 2026-09-24，已发布 81 条技术信号、16 条技能、19 条知识、30 期技术简报，
在用信源 13 个；内容关系图谱共 116 个节点、802 条带类型的边。2026-04 的 8 条演示用
种子信号已于 2026-09-24 归档，不再出现在任何公开页面。
...
兜底类型「关联」约占全部边的 13%。同一张图谱用于信号页的版本脉络、话题页，以及 AI
生成学习路径时的依据。
